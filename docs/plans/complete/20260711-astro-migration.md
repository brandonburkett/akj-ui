...
- [x] EXTRA (user request): colocate single-use modules into their component folders — `42a506e` (8 renames + 4 `.astro` import edits + vitest coverage repoint; check/build/24 unit+14 e2e/coverage green) — `src/lib/seo.ts`(+`seo.test.ts`)→`components/SeoHead/`, `src/scripts/{nav,parallax,gallery}.ts`(+tests)→each component's folder. Simplifies the `.astro` imports to `./seo`/`./nav`/etc; remove the now-empty `src/lib` + `src/scripts`; repoint `vitest.config.ts` coverage `include` (e.g. `src/components/**/*.ts` minus tests). Verify check/build/test green.
...
- [x] INJECTED (user requests, post-Task-13; design specs written during dev, removed before merge; all check + build (6 pages) + 28 unit + 15 e2e green, cross-browser confirmed where visual):
...
- [x] FINAL REVIEW: focused whole-branch review on opus (deep review of the injected and pre-review work, plus a whole-branch coherence and security sweep). Verdict READY TO MERGE, 0 critical, 0 important. Minors handled (stale react-snap comment reworded, production.sh robustness notes logged). Pre-review changes folded in first: SeoHead rel=icon and drop title lang, gallery prefers-reduced-motion gating, README Follow-ups section.
- [x] PRE-PR DECISIONS with the user:
  - Service worker: keep manifest-only installability, offline service worker logged as a README follow-up.
  - ESLint: skipped. astro check runs tsc in CI and covers type errors, no separate lint layer for a static site this size.
  - Deferred to the README Follow-ups: nav Popover API, astro:assets, @astrojs/sitemap, full Lightning CSS migration, SEO audit, broader dead-CSS sweep.
- [x] ASTRO 7 UPGRADE (after a dry-run showed it near-trivial): astro 6.4.8 to 7.0.7 and @astrojs/check to 0.9.9. Removed 8 dead IE star-hacks (v7 lightningcss minifier hard-errors on them). Fixed a v7 CSS-pipeline regression where the new default lightningcss minifier stripped autoprefixer's prefixes (notably -webkit-text-size-adjust for iOS), by pinning vite.build.cssMinify to esbuild to restore the verified v6 output. Node 24.18.0 already satisfies v7. CircleCI and production.sh build and S3 paths verified correct for v7.
- [x] DEV-TOOLING BUMPS: stylelint 16 to 17, stylelint-config-standard 36 to 40, lint-staged 15 to 17. TypeScript held at 5.x (7.0 is the brand-new native rewrite, astro-check compat risk). config-standard 40 handled (regex ignore for the kept -webkit-appearance idioms, ignore clip for the .visuallyhidden a11y pattern, safe modernizations for overflow-wrap and break-inside/after). npm audit 0 vulns.
- [x] POLISH: removed stray cursor:pointer on nav .site-menu, CLAUDE.md note to run check and lint:style before committing, pretest:e2e hook to auto-install Chromium plus README setup docs.
- [ ] REMAINING: push the branch and open the squash-merge PR into master.
...
- **Autoprefixer / Lightning CSS.** ✅ DONE via the autoprefixer path (`0dc0c56`+`3d78b9c`, redundant-override cleanup `95bd320`; cross-browser verified). The three `*-no-vendor-prefix` rules are re-enabled; Lightning CSS remains a possible future swap. Original note: [from Task 13] Astro does not autoprefix by default (since v0.21); CRA's react-scripts DID, driven by the same `browserslist` we kept. So the Astro build ships only the vendor prefixes hand-written in the source CSS, potentially fewer than the old CRA output for auto-prefixable properties (small gap in 2026, but a real difference from the old build). To restore/modernize, either (a) `npm i -D autoprefixer` + a `postcss.config.cjs` (classic, auto-detected by Vite, uses the existing browserslist), or (b) `npm i -D lightningcss` + `vite.css.transformer: 'lightningcss'` + targets (faster, also minifies). Astro has no mandated best practice here (both are first-class) and already minifies CSS in production, so SKIP `cssnano` (the docs example includes it but it is redundant) — a PostCSS config needs only `autoprefixer`. Recommendation for this legacy CSS: PostCSS + autoprefixer (lower risk, matches CRA, minimal pipeline change); reserve Lightning CSS for a greenfield project or dedicated follow-up. Payoff: could then delete the hand-written prefixes from source and re-enable the three `*-no-vendor-prefix` stylelint rules disabled in Task 13. Post-migration, not a blocker.
...
- **Broader dead-CSS sweep.** [from the dead-selector cleanup] The selector-pattern cleanup (`d6e4950`) removed 32 dead selectors (`master.css` −323 lines), but only the ones that tripped the naming rules. `master.css` almost certainly has MORE dead CSS with kebab-case names (old-app base styles: `.card`, `.notice`, `.teaser`, screenshot/table/interior-page styles) that a naming-rule audit can't surface. A thorough pass: for every selector, grep all reference paths (`.astro`/`.ts`/`.json` incl. `set:html` strings); delete the unreferenced rules (handle selector lists + `@media` dupes); verify build + full cross-browser. Bigger and riskier than the targeted cleanup — its own task.
